...
def _json_safe(obj: Any) -> str:
    """JSON serializer that handles datetime and date objects from Kite API."""
...
def _extract_risky_action(
    ...
    if not symbol_match:
        inferred_symbol = _infer_tradingsymbol(message)
        if inferred_symbol:
            arguments["tradingsymbol"] = inferred_symbol
...
def _infer_tradingsymbol(message: str) -> Optional[str]:
    lowered = message.lower()

    patterns = (
        r"\b(?:of|for|buy|sell|place|order)\s+([a-z][a-z0-9&.-]{1,19})\b",
        r"\b([a-z][a-z0-9&.-]{1,19})\s+(?:share|shares|stock|stocks|equity)\b",
    )
    ...
    words = re.findall(r"\b[a-z][a-z0-9&.-]{1,19}\b", lowered)
    for word in reversed(words):
        if word not in _SYMBOL_STOPWORDS:
            return word.upper()
